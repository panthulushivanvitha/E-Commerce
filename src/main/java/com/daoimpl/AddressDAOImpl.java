...
private static Logger log = LoggerFactory.getLogger(AddressDAOImpl.class);
	...
	public Address getAddressById(int addressId) {
		...
		Address address=(Address) sessionFactory.getCurrentSession().createQuery("select a from Address a where a.id = :addressId").setParameter("addressId", addressId).uniqueResult();
	...
	public List<Address> getAllAddressOfUser(Integer userId) {
		...
		cq.select(addressRoot).where(cb.equal(addressRoot.get("personEmail"), userId));
